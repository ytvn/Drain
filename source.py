...
if __name__ == '__main__':
    ...
    for i in range(len(full_names)):
        ...
        for step in range(2, 77):
            print("\n=== No: %s Device: %s Seq step: %s ===" % (str(i), full_names[i], str(step)))
            ...
            model = LSTMAutoencoder(dataset=dataset, dataset_name=names[i] + "/step" + str(step), epochs=50, batch=32,
                                    lr=0.1)
            ...
            error_df, pred_time = model.evaluation_metric(dataset.X_test, dataset.y_test)
            # print("Predict time: %s" % (str(pred_time)))
            error_df.dropna(inplace=True)
            if error_df.shape[0] < 10:
                continue

            model.ROC(error_df=error_df)
            threshold_rt = model.precision_recal_curve(error_df)

            max_accuracy, thres, thres_exe = model.best_accuracy(error_df=error_df, threshold_rt=threshold_rt)
            # print("Brute force threshold time: %s" % (str(thres_exe)))

            model.confusion_matric(thres, error_df)
            model.reconstruction_error_for_2_class(thres, error_df)

            d = {"DatasetName": names[i], "Step": step, "Threshold": thres, "Accuracy": max_accuracy, "Bruteforce time": thres_exe}
            df = pd.DataFrame([d])
